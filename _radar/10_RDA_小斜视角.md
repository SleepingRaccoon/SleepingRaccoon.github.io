...
*   \( w_r\left[\tau - \dfrac{2R(\eta)}{c}\right] \) 和 \( \exp\left\{j \pi K_r \left(\tau - \dfrac{2R(\eta)}{c}\right)^2\right\} \)：**被时延整体搬移的一段脉冲**。距离窗只是这段 LFM 的"存在范围"，LFM 的中心和窗的中心都在 \( \tau = 2R(\eta)/c \) 上，两者由同一个时延一起搬移。所以要把它们当一个整体：随 \( \eta \) 缓慢移动的就是这一整段脉冲。距离压缩能把这个"块"压成尖峰，但**压不掉它在慢时间上的移动**——这正是后面距离徙动的来源，也是 RCMC 必须存在的理由。
*   \( \exp\left\{-j \dfrac{4\pi f_0 R(\eta)}{c}\right\} \)：**载频相位**。它随慢时间变化，是多普勒（方位向 LFM）的来源。

距离压缩在频域完成：对 \( \tau \) 做 FFT，乘以频域匹配滤波器 \( H(f_\tau) = \exp\{j \pi f_\tau^2 / K_r\} \)，再 IFFT 回时域。

**POSP 推导驻定点。** 对 \( \tau \) 做 FFT，需要计算积分 \( \int s_0(\tau,\eta) \exp\{-j 2\pi f_\tau \tau\} d\tau \)。总相位为
\[
...
这个积分的驻定相位形式写全了是
\[
 \int s_0(\tau,\eta) e^{-j 2\pi f_\tau \tau} d\tau \;\approx\; A_0 W_r(f_\tau)\, \exp\{\, j\Theta(\tau^{*}) \,\} \sqrt{\frac{2\pi}{|\Theta''|}} \exp\left\{ j\frac{\pi}{4} \operatorname{sgn}(\Theta'') \right\}
\]
其中三项分别是：
*   \( W_r(f_\tau) \)：距离窗 \( w_r \) 经驻点 \( \tau^{*} \) 尺缩后的频域形式。窗的自变量在驻点处取值 \( \left.(\tau - 2R/c)\right|_{\tau=\tau^{*}} = f_\tau/K_r \)，所以 \( w_r\left[\tau - 2R(\eta)/c\right] \) 变成 \( w_r(f_\tau/K_r) \)，记作 \( W_r(f_\tau) \)。
...
\[
 S_0(f_\tau, \eta) = A_0 W_r(f_\tau) \exp\left\{-j \pi \frac{f_\tau^2}{K_r}\right\} \exp\left\{-j \frac{4\pi f_\tau R(\eta)}{c}\right\} \exp\left\{-j \frac{4\pi f_0 R(\eta)}{c}\right\} \tag{6}
\]
乘上匹配滤波器后二次相位被抵消：
\[
 S_0(f_\tau, \eta) H(f_\tau) = A_0 W_r(f_\tau) \exp\left\{-j \frac{4\pi f_\tau R(\eta)}{c}\right\} \exp\left\{-j \frac{4\pi f_0 R(\eta)}{c}\right\} \tag{7}
\]

**IFFT 回时域。** 对 \( f_\tau \) 做 IFFT，需要计算积分 \( \int S_0 H \exp\{j 2\pi f_\tau \tau\} df_\tau \)。二次相位已经被匹配滤波抵消，剩下的相位对 \( f_\tau \) 是线性的，没有驻点，直接积分；只是 \( W_r(f_\tau) \) 没有具体形式，这个积分写不出解析式，直接把它记成距离向冲激响应 \( p_r \)：
\[
 p_r\left[\tau - \frac{2R(\eta)}{c}\right]
 = \int W_r(f_\tau) \exp\left\{j 2\pi f_\tau \left[\tau - \frac{2R(\eta)}{c}\right]\right\} df_\tau \tag{8}
\]
压缩后信号为
\[
 s_{rc}(\tau, \eta) = A_0 p_r\left[\tau - \frac{2R(\eta)}{c}\right] w_a(\eta - \eta_c) \exp\left\{-j \frac{4\pi f_0 R(\eta)}{c}\right\} \tag{9}
\]
\( p_r \) 的形状由窗函数决定，矩形窗下是 \( \operatorname{sinc}(B_r\tau) \)，常数因子并进 \( A_0 \)。压缩后的距离分辨率为 \( \rho_r = \frac{c}{2} \frac{0.886 \gamma_{w,r}}{|K_r| T_r} \)，其中 \( T_r \) 为脉冲宽度，\( K_r T_r \) 为带宽。
...
\]
记方位调频率 \( K_a = \frac{2V_r^2}{\lambda R_0} \)，方位向信号在时域就是一个线性调频信号 \( \exp\{-j \pi K_a \eta^2\} \)。

**POSP 推导驻定点。** 对 \( \eta \) 做 FFT，需要计算积分 \( \int s(\eta) \exp\{-j 2\pi f_\eta \eta\} d\eta \)。总相位为
\[
...
\[
 \int s(\eta) e^{-j 2\pi f_\eta \eta} d\eta \;\approx\; A_0 W_a(f_\eta - f_{\eta_c})\, \exp\{\, j\Theta(\eta^{*}) \,\} \sqrt{\frac{2\pi}{|\Theta''|}} \exp\left\{ j\frac{\pi}{4} \operatorname{sgn}(\Theta'') \right\}
\]
其中三项分别是：
...
\[
 S_1(\tau, f_\eta) = A_0 p_r\left[\tau - \frac{2R_{rd}(f_\eta)}{c}\right] W_a(f_\eta - f_{\eta_c}) \exp\left\{-j \frac{4\pi f_0 R_0}{c}\right\} \exp\left\{j \pi \frac{f_\eta^2}{K_a}\right\} \tag{16}
\]
瞬时斜距在频域表现为
\[
...
**做法二：距离频域相位相乘。** 傅里叶变换的时移性质是 \( x(\tau - \tau_0) \leftrightarrow X(f_\tau) \exp\{-j 2\pi f_\tau \tau_0\} \)。要实现距离偏移 \( 2\Delta R(f_\eta)/c \)，在距离频域乘
\[
 G_{rcmc}(f_\tau) = \exp\left\{j \frac{4\pi f_\tau \Delta R(f_\eta)}{c}\right\} \tag{20}
\]
...
\[
 S_2(\tau, f_\eta) = A_0 p_r\left(\tau - \frac{2R_0}{c}\right) W_a(f_\eta - f_{\eta_c}) \exp\left\{-j \frac{4\pi f_0 R_0}{c}\right\} \exp\left\{j \pi \frac{f_\eta^2}{K_a}\right\} \tag{21}
\]

**方位匹配滤波。** 取式(21)中第二个指数项的复共轭作为滤波器：
\[
 H_{az}(f_\eta) = \exp\left\{-j \pi \frac{f_\eta^2}{K_a}\right\} \tag{22}
\]
相乘后频域相位被补偿干净：
\[
 S_3(\tau, f_\eta) = S_2(\tau, f_\eta) H_{az}(f_\eta) = A_0 p_r\left(\tau - \frac{2R_0}{c}\right) W_a(f_\eta - f_{\eta_c}) \exp\left\{-j \frac{4\pi f_0 R_0}{c}\right\} \tag{23}
\]

**方位 IFFT 与最终图像。** 对 \( f_\eta \) 做 IFFT，此时相位对 \( f_\eta \) 是线性的，没有驻点，直接积分。\( W_a(f_\eta - f_{\eta_c}) \) 对应时域 \( w_a(\eta) \exp\{j 2\pi f_{\eta_c} \eta\} \)，于是得到二维压缩图像
\[
 s_{ac}(\tau, \eta) = A_0 p_r\left(\tau - \frac{2R_0}{c}\right) p_a(\eta) \exp\left\{-j \frac{4\pi f_0 R_0}{c}\right\} \exp\{j 2\pi f_{\eta_c} \eta\} \tag{24}
\]
其中 \( p_a(\eta) \) 是方位冲激响应。
*   包络 \( p_r \) 把目标聚焦在 \( \tau = 2R_0/c \)，而且与 \( f_\eta \) 无关，说明 RCM 已经校正掉。
*   包络 \( p_a \) 把目标聚焦在 \( \eta = 0 \)，也就是零多普勒位置。残余的线性相位 \( \exp\{j 2\pi f_{\eta_c} \eta\} \) 在 \( \eta = 0 \) 处为零；斜视时这一项不为零，做干涉、极化这类需要保留相位关系的处理时要先补偿掉。
