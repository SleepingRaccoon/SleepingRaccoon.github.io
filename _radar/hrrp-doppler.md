...
*   **参考距离限制**：$R_{ref}$ 必须选在目标场景中心。如果 $R_{ref}$ 偏离目标太远，差频频率 $f_{if} = \frac{2\gamma}{c}\lvert R_{\Delta i} \rvert$ 会超出ADC的低通滤波带宽，导致回波被滤除。
...
一个常见的疑问是：为什么不写成 $s_{ref} \cdot s_r^\ast $，那样相位里直接就是 $R_i - R_{ref}$，看着更顺？
实际上**标准工程做法是回波乘参考的共轭，即 $s_r \cdot s_{ref}^\ast $**，推导一遍就知道差在哪：
...
**结论**：采用 $s_r \cdot s_{ref}^\ast $，差频频率 $f_{if} = -\frac{2\gamma}{c} R_{\Delta i}$。如果目标比参考近（$R_{\Delta i} < 0$），$f_{if} > 0$，这是一个正频率，方便FFT处理。如果反过来用 $s_{ref} \cdot s_r^\ast $，频率会变成负的，导致频率轴翻转。因此，标准做法是回波乘参考的共轭。
...
通过上述严格推导，宽带目标的RD图（$\lvert S(f, f_d) \rvert$）具有以下特征：
