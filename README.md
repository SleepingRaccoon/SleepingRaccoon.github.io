...
- **行内公式**：`$f_{IF}$` —— 推荐这一种。本站渲染为行内，VS Code / GitHub / Obsidian
  的 Markdown 预览也认。写在同一行的 `$$f_{IF}$$` 本站同样按行内渲染，
  但多数编辑器预览会把它当成**行间**公式，两边看起来就对不上了。
- **独立公式**（`$$` 单独占一行，前后各留一个空行）：
...
- kramdown 只把 `$$...$$` 当作"数学区"（内容原样保留），构建时转成 MathJax 认识的
  `\(...\)` / `\[...\]`；`$...$` 则是普通文本，由 `_layouts/default.html` 里的 MathJax 兜住渲染。
  两者在本站都能正常显示，差别在于公式里能用什么字符：
  `$$...$$` 里的内容不会被 markdown 处理，`$...$` 里的反斜杠转义会被 kramdown 吃掉，
  所以 `$...$` 里不要写 `\_`、`\{` 这类转义，撒号写成 `\prime` 而不是 `'`
  （智能引号会把 `'` 改成弯引号）。
